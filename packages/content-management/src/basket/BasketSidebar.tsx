...
export const BasketSidebar: React.FC<BasketSidebarProps> = ({ steps }) => {
  const { state, setCurrentStep, toggleSidebar, nextStep } = useBasket();
  ...
  return (
    <aside className={`h-screen border-r border-slate-200 bg-white transition-all duration-300 flex flex-col relative z-50 ${state.isSidebarCollapsed ? 'w-20' : 'w-72'}`}>
      <div className="p-5 flex items-center justify-between">
        {!state.isSidebarCollapsed && <span className="font-bold text-slate-800">Basket Management</span>}
              ...
              {!isLastStepInList && (
                <div 
                  ...
                  style={{
                    ...
                    borderLeft: `2px dotted ${isDone ? '#8b5cf6' : '#e2e8f0'}`,
                  }}
                />
              )}
              <div className={`z-10 w-8 h-8 rounded-full flex items-center justify-center transition-all ${isActive ? 'bg-purple-500 text-white shadow-lg shadow-purple-500/30 scale-110' : isDone ? 'bg-purple-50 text-purple-500' : 'bg-slate-100 text-slate-400'}`}>
                {isDone ? <CheckCircle2 size={16} /> : <span className="text-xs font-bold">{idx + 1}</span>}
              </div>
              {!state.isSidebarCollapsed && (
                <div className="ml-4 overflow-hidden">
                  <p className={`text-xs font-bold ${isActive ? 'text-purple-500' : 'text-slate-600'}`}>{step.title}</p>
        ...
        {state.isSidebarCollapsed ? (
          <div className="flex flex-col gap-2 items-center">
            <button
              ...
              className="w-10 h-10 rounded-lg flex items-center justify-center transition-all disabled:opacity-50 disabled:cursor-not-allowed bg-purple-500 text-white hover:bg-purple-600 disabled:hover:bg-purple-500"
              title="Continue"
            >
          ...
          <>
            <button 
              className="w-full py-2 px-4 bg-purple-500 text-white rounded-lg hover:bg-purple-600 disabled:opacity-50 disabled:cursor-not-allowed transition-all font-medium text-sm flex items-center justify-center" 
              disabled={isOnLastStep} 
              onClick={handleNext}
            >
