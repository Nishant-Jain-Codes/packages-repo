...
export const BlockSidebar: React.FC<BlockSidebarProps> = ({ steps }) => {
  const { state, setCurrentStep, toggleSidebar, nextStep } = useBlock();
  ...
  return (
    <aside className={`h-screen border-r border-slate-200 bg-white transition-all duration-300 flex flex-col relative z-50 ${state.isSidebarCollapsed ? 'w-20' : 'w-72'}`}>
      <div className="p-5 flex items-center justify-between">
        {!state.isSidebarCollapsed && <span className="font-bold text-slate-800">Block Management</span>}
              ...
              {!isLastStepInList && (
                <div 
                  ...
                  style={{
                    ...
                    borderLeft: `2px dotted ${isDone ? '#22c55e' : '#e2e8f0'}`,
                  }}
                />
              )}
              <div className={`z-10 w-8 h-8 rounded-full flex items-center justify-center transition-all ${isActive ? 'bg-green-500 text-white shadow-lg shadow-green-500/30 scale-110' : isDone ? 'bg-green-50 text-green-500' : 'bg-slate-100 text-slate-400'}`}>
                {isDone ? <CheckCircle2 size={16} /> : <span className="text-xs font-bold">{idx + 1}</span>}
              </div>
              {!state.isSidebarCollapsed && (
                <div className="ml-4 overflow-hidden">
                  <p className={`text-xs font-bold ${isActive ? 'text-green-500' : 'text-slate-600'}`}>{step.title}</p>
        ...
        {state.isSidebarCollapsed ? (
          <div className="flex flex-col gap-2 items-center">
            <button
              ...
              className="w-10 h-10 rounded-lg flex items-center justify-center transition-all disabled:opacity-50 disabled:cursor-not-allowed bg-green-500 text-white hover:bg-green-600 disabled:hover:bg-green-500"
              title="Continue"
            >
          ...
          <>
            <button 
              className="w-full py-2 px-4 bg-green-500 text-white rounded-lg hover:bg-green-600 disabled:opacity-50 disabled:cursor-not-allowed transition-all font-medium text-sm flex items-center justify-center" 
              disabled={isOnLastStep} 
              onClick={handleNext}
            >
